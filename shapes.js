...
function Shapes() {
	...
	//for each plot.
	this.frequencyBins = ["sphere", "lowMid", "highMid", "treble"];
	...
	this.draw = function() {
		...
		for (var i = 0; i < this.plotsDown; i++) {
			for (var j = 0; j < this.plotsAcross; j++) {
				...
				var x = 50 + this.plotWidth *j;
				var y = 50 + this.plotHeight *i;
	...
	this.needle = function(energy, centreX, bottomY) {
		...
		theta = map(energy, 0, 255, minRadius, maxRadius);
